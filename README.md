...
**Built-in adapters:** OpenAI · Azure OpenAI · Anthropic · Microsoft Presidio.
Any other stack (FastAPI, LangChain, batch jobs, your own SDK) plugs in via the
...
Highlight, classify, and redact PII in the browser — public API only, **zero
external calls**. Editor + per-category highlighting, detection cards, JSON view,
redaction preview, synthetic Arabic samples, `.txt` upload, light/dark.
...
Details in [playground/README.md](playground/README.md).
...
Prefer the shell? `echo "National ID 1158813996" | tabayyan scan -` — full [CLI](#cli) below.
